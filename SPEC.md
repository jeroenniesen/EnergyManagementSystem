...
**Endpoints (FastAPI):** `GET /api/status` (intent, ownership, badges), `GET /api/plan`, `GET /api/series?metric=…`, `GET/POST /api/settings` (token-protected, **same-origin/CSRF-checked**, §12), `GET /api/freshness` (per-signal), `GET /api/diagnostics` (why-not), `GET /api/incidents` (control-health rollup), `POST /api/control/{return_default|pause|force_charge_target|override}`, `GET /api/energy-distribution` (a day's Sankey), `GET /api/report?period=&date=` (Insights, §9.1.1), `GET /api/export/{plan|measurements|weekly|package}`, `GET /api/setup/checks`, `WS /ws`. **Health:** `GET /health/live`, `GET /health/ready` (§11).
