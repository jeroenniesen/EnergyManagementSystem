...
**Status:** design v4 (approach approved 2026-07-25: **refuse-when-busy** self-restart) · **Author:** Fable 5 · **Cross-family spec review:** Sol — v1/v2/v3 CHANGES-REQUIRED (converged to 2 precise predicate fixes), v4 pending · **Base:** `main` @ `f6791ae` · **Branch:** `feat/apply-restart-auth-styling`
...
2. **No control-cycle worker outstanding.** Relying on `control_lock` is NOT sufficient: `run_cycle` releases the lock after its `wait_for(to_thread(control_tick))` times out, while the non-killable worker may still be running and has not yet reached the battery write (`service.py:1310/1318`, write at `~1219`). I2 tracks the control-tick worker as a **strongly-referenced `asyncio.Task` wrapping `asyncio.to_thread(control_tick)`, awaited via `asyncio.shield`** so a `wait_for` timeout does NOT mark it done. Idle requires this task is absent or truly `.done()`. It is cleared **only on real worker completion**, never in the timeout handler.
3. **No override cycle task pending** — the tracked override tasks (`service.py:327`, `api.py:2266/2292`) are all done; and the direct overrun-AUTO write (`service.py:1382`) is covered by the `write_in_flight` counter (5).
4. **Last command confirmed AND AUTO.** Not merely `last_confirmed_action in {AUTO, None}`: a timed-out / `BatteryWriteUnconfirmed` forced write leaves `last_confirmed_action` stale (`mode_controller.py:341/353`) even though the driver says such a write likely reached the device (`indevolt_driver.py:241`). I2 adds a **persisted "last-command unconfirmed/unknown" flag**, set when a write returns unconfirmed, cleared **only on a confirmed SetData acceptance** (incl. confirmed AUTO recovery — the existing confirmation model, no new physical cluster read). Idle requires: **NOT unconfirmed** AND `last_confirmed_action in {AUTO, None}`. Dry-run / not-operational always satisfies this (driver never armed).
5. **`write_in_flight` is zero.** A **thread-safe counter/token** (not a bool — post-timeout overlapping writes are possible), incremented **on the worker thread immediately around the real driver call** and decremented in a `finally`, covering the primary write (`mode_controller.py:340`), the AUTO recovery (`:360`), and the overrun-AUTO write (`service.py:1382`). Idle requires zero. (Belt-and-suspenders alongside (2).)

**No-yield snapshot.** The reads for (1)-(5) and the set of `_restart_requested`/`draining` must be ONE synchronous, no-`await` operation (there is no `asyncio.Lock.acquire_nowait`; use `control_lock.locked()` + the task-`.done()` + flag/counter reads synchronously, then set the flags — no yield in between), so nothing changes between the check and the decision.
...
- **Idle-and-safe (the safety core):** 409 with `reason` when — (a) the control-tick worker `asyncio.Task` is still pending, INCLUDING a **timed-out worker that has not yet reached `driver.apply`** (simulate a `to_thread` that outlives the `wait_for`; the task must read pending and refuse); (b) the persisted **"last-command unconfirmed" flag** is set from a prior `BatteryWriteUnconfirmed` forced write EVEN WHEN `last_confirmed_action` is AUTO/None (this is the case the naive predicate wrongly accepted); (c) an override task is pending; (d) `write_in_flight` counter > 0; (e) `last_confirmed_action` is a non-AUTO mode. **202 + `request_restart` spy invoked + audit row** only when ALL hold (incl. dry-run/not-operational → always AUTO-safe). Prove the idle snapshot + flag-set is synchronous/no-yield (single-flight can't double-fire; nothing changes between check and decision). Prove the worker-outstanding flag is cleared ONLY on real completion, never in the timeout handler.
...
- **I2 — Restart backend** (**Opus** → Sol; safety-critical): the outstanding-control-worker task tracking (shielded `asyncio.Task`, cleared only on real completion) + the persisted "last-command unconfirmed" flag (cleared only on confirmed acceptance) + the thread-safe `write_in_flight` counter (on the worker thread) + `draining` flag + the no-yield idle-and-safe computation; the endpoint (register-only-with-auth, ADMIN+session, strict `_is_supervised`, single-flight, response-first injectable SIGTERM trigger, no `os._exit`); add the endpoint to the write-gating invariant; `boot_id`; server `restart_available`/`restart_pending` (post-load fingerprint); install.sh/compose env; backend tests incl. the uvicorn subprocess test.
